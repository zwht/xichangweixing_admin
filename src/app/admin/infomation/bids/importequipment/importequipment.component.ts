...
import { BidsService } from 'src/app/share/restServices/bids.service';
...
export class ImportBidsComponent implements OnInit {
  excle = '';
  dataHead = [];
  dataMessage = [];
  fuckK = true;
  ...
  constructor(
    ...
    private supplierService: BidsService,
    ...
    this.supplierService['importData']({
      ...
      .subscribe(response => {
        if (response.errorCode === 0) {
          ...
          this.router.navigate(['admin/infomation/bids']);
        } else {
  ...
  down() {
    ...
    window.open('/v1/tenderOrgation/getExport?Authorization=' + aaa);
  }
